...
class WPML_Media_Attachments_Query implements IWPML_Action {
	...
	public function adjust_attachment_query( $query ) {
		if ( isset( $query->query['post_type'] ) && 'attachment' === $query->query['post_type'] && ( ! isset( $query->query_vars['force_suppress_filters'] ) || true !== $query->query_vars['force_suppress_filters'] ) ) {
