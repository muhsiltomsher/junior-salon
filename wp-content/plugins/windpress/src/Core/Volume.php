...
class Volume
{
    public static function get_entries() : array
    {
        ...
        foreach ($finder as $file) {
            ...
            $entries[] = ['name' => $file->getFilename(), 'relative_path' => $file->getRelativePathname(), 'content' => $file->getContents(), 'handler' => 'internal', 'signature' => \wp_create_nonce(\sprintf('%s:%s', WIND_PRESS::WP_OPTION, $file->getRelativePathname())), 'readonly' => \strpos(\wp_normalize_path($file->getPathname()), \wp_normalize_path($data_dir)) === \false];
        }
