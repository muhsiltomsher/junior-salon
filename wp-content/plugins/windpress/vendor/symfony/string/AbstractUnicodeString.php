...
use WindPressDeps\Symfony\Component\String\Exception\ExceptionInterface;
use WindPressDeps\Symfony\Component\String\Exception\InvalidArgumentException;
use WindPressDeps\Symfony\Component\String\Exception\RuntimeException;
/**
...
abstract class AbstractUnicodeString extends AbstractString
{
    ...
    {
        $str = clone $this;
        if (!$compat || \PHP_VERSION_ID < 70300 || !\defined('WindPressDeps\\Normalizer::NFKC_CF')) {
    ...
    {
        $str = $this->camel();
        $str->string = \mb_strtolower(\preg_replace(['/(\\p{Lu}+)(\\p{Lu}\\p{Ll})/u', '/([\\p{Ll}0-9])(\\p{Lu})/u'], 'WindPressDeps\\1_\\2', $str->string), 'UTF-8');
        return $str;
    }
