...
import { oxyIframe as i } from "./constant-DEJBDgok.js";
Promise.all([
  (() => {
    try {
      return __tla_0;
...
`;
    ...
    let c;
    document.head.querySelector(`style#windpress-variables`) ? c = document.head.querySelector(`style#windpress-variables`) : (c = document.createElement(`style`), c.id = `windpress-variables`, document.head.appendChild(c)), c && (c.textContent = s);
    let l;
    i.contentWindow.document.head.querySelector(`style#windpress-variables`) ? l = i.contentWindow.document.head.querySelector(`style#windpress-variables`) : (l = i.contentWindow.document.createElement(`style`), l.id = `windpress-variables`, i.contentWindow.document.head.appendChild(l)), l && (l.textContent = s);
  }
