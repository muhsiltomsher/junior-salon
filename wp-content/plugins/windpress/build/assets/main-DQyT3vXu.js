...
import { Buffer as e, init_dist$1 as t } from "./dist-BSmntxBi.min.js";
...
import { decodeVFSContainer as n } from "./vfs-BzllAKLZ.js";
import { getVariableList as r, loadDesignSystem as i } from "./intellisense-5fH2Lpgq.js";
import "./lib-DdIaIIpq.min.js";
import { __tla as __tla_2 } from "./tailwindcss-NwOviTEG.js";
import { logger as a } from "./logger-B1Q-9Dmh.js";
import { brxGlobalProp as o, brxIframe as s } from "./constant-B9ERhtGk.js";
Promise.all([
  (() => {
    try {
      return __tla_0;
  ...
  function u(t2) {
    if (e.isBuffer(t2)) return t2;
    if (typeof t2 == `number`) return e.alloc(t2);
    if (typeof t2 == `string`) return e.from(t2);
    throw Error(`input must be buffer, number, or string, received ` + typeof t2);
  }
  function d(e2) {
    let t2 = u(4);
    return t2.writeInt32BE(e2, 0), t2;
  }
  function f(t2, n2) {
    t2 = u(t2), e.isBuffer(n2) && (n2 = n2.readUInt32BE(0));
    let r2 = ~~n2 ^ -1;
    for (var i2 = 0; i2 < t2.length; i2++) r2 = l[(r2 ^ t2[i2]) & 255] ^ r2 >>> 8;
    return r2 ^ -1;
  }
  function p() {
    return d(f.apply(null, arguments));
  }
  p.signed = function() {
  ...
  const h = c(m);
  function g(e2) {
    let t2 = h(e2).toString(`hex`);
    return `windpress-${t2}`;
  }
  async function _() {
    o.$_state.colorPalette = o.$_state.colorPalette.filter((e3) => e3.name !== `windpress`);
    let e2 = s.contentWindow.document.querySelector(`script#windpress\\:vfs[type="text/plain"]`), t2 = n(e2.textContent), a2 = await r(await i({
      volume: t2
    })), c2 = [];
    a2.filter((e3) => e3.key.startsWith(`--color`)).forEach((e3) => {
      c2.push({
        id: g(e3.key),
        raw: `var(${e3.key})`,
        name: e3.key.substring(2)
      });
    }), o.$_state.colorPalette.find((e3) => e3.name === `windpress`) || o.$_state.colorPalette.push({
      id: `windpress`,
      name: `windpress`,
      colors: c2,
      default: true
  ...
  const v = new BroadcastChannel(`windpress`);
  v.addEventListener(`message`, async (e2) => {
