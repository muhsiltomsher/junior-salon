...
add_action('wp_ajax_fetch_sorted_products_by_category', 'fetch_sorted_products_by_category');
add_action('wp_ajax_nopriv_fetch_sorted_products_by_category', 'fetch_sorted_products_by_category');

function fetch_sorted_products_by_category() {
    $sort     = isset($_GET['sort']) ? sanitize_text_field($_GET['sort']) : '';
    $page     = isset($_GET['page']) ? intval($_GET['page']) : 1;
    ...
    // Apply taxonomy filter if term_id is valid
    if ($term_id) {
