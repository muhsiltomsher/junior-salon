...
<div class="overflow-hidden flex flex-col relative group">
    <div class="relative block overflow-hidden group">
        ...
        <!-- Wishlist Icon -->
        <button class="absolute top-2 right-2 z-10 wishlist-toggle" aria-label="Add to wishlist">
            <svg class="w-6 h-6 transition-all duration-300 fill-white stroke-2 stroke-gray-700 hover:stroke-pink-500"
                viewBox="0 0 24 24" fill="none">
                <path
                    d="M12 21s-6.716-5.165-9-9c-2-3.333 1-8 6-6 2 1 3 3 3 3s1-2 3-3c5-2 8 2.667 6 6-2.285 3.835-9 9-9 9z" />
            </svg>
        </button>
        ...
        ?>
        <div
            class="absolute bottom-0 left-0 right-0 bg-white/95 text-black py-3 px-[10px] opacity-0 translate-y-full group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 z-20">
            ...
            <div class="flex flex-wrap justify-center gap-[8px] variation-hover-form mb-[10px]"
                ...
                <label class="cursor-pointer transition">
                    ...
                    <span
                        class="px-[10px] py-1 border border-gray-400 rounded-full text-[10px] uppercase text-black peer-checked:bg-[#DEAF27] peer-checked:text-white peer-checked:border-[#DEAF27] transition">
            ...
            <!-- Add to Cart / Buy Now Buttons (triggered via JS) -->
            <div class="flex justify-center gap-2 mt-2">
                <form class="variation-cart-form" method="post">
                    <input type="hidden" name="variation_id" value="">
                    <button type="submit"
                        class="text-xs uppercase px-4 py-1.5 bg-black text-white border border-black hover:bg-transparent hover:text-black transition add-to-cart-btn"
    ...
    <h2 class="text-xs text-center font-medium mt-1 !text-gray-800">
        <a href="<?php the_permalink(); ?>"
            class="!text-gray-800 no-underline hover:underline transition-colors duration-200">
            <?php the_title(); ?>
        </a>
    </h2>
...
jQuery(document).ready(function($) {
    ...
    // Store variation ID on selection
    $(document).on('change', '.variation-hover-form input[type="radio"]', function() {
        ...
        if (matched) {
            selectedVariationId = matched.variation_id;
            $form.closest('.variation-cart-form').find('input[name="variation_id"]').val(selectedVariationId);
        }
    });
    ...
    $(document).on('click', '.wishlist-toggle', function() {
        $(this).find('svg').toggleClass('fill-pink-500');
    });
});
</script>
