...
<div class="overflow-hidden flex flex-col relative group border border-gray-200">
  ...
  <!-- Attributes will be injected by JS -->
  <button type="submit"
...
jQuery(function($) {
  ...
  $(document).on('change', '.variation-hover-form input[type="radio"]', function () {
    const form = $(this).closest('.variation-cart-form');
    const variationForms = form.closest('.group').find('.variation-hover-form');
    const variations = variationForms.first().data('variations');

    let selectedAttributes = {};
    variationForms.each(function () {
      const attrKey = $(this).data('attr-key');
      const val = $(this).find('input[type="radio"]:checked').val();
      if (val) {
        selectedAttributes[`attribute_${attrKey}`] = val.trim().toLowerCase();
      }
    });
    ...
    const matched = variations.find(v => {
      return Object.entries(v.attributes).every(([key, val]) => {
        return (
          ...
          selectedAttributes[key].toLowerCase() === val.toLowerCase()
        );
      });
    });

    if (matched) {
      form.find('input[name="variation_id"]').val(matched.variation_id);
    } else {
  ...
  $(document).on('click', '.wishlist-toggle', function() {
    ...
    // Animation
    $svg.addClass('scale-150 transition-transform duration-150');
    setTimeout(() => $svg.removeClass('scale-150'), 150);

    if (isActive) {
      $svg.removeClass('text-pink-500 stroke-pink-500')
          .addClass('text-transparent stroke-black');
      $btn.removeClass('border-pink-500').addClass('border-gray-300');
    } else {
      $svg.removeClass('text-transparent stroke-black')
          .addClass('text-pink-500 stroke-pink-500');
