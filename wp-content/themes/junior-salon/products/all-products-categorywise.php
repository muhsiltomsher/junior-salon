...
jQuery(document).ready(function($) {
    let page = 1;
    let currentSort = '';
    let currentTaxonomy = 'product_cat'; 
    $('#drawer').on('change', 'input[name="sort"]', function() {
        currentSort = $(this).val();
        page = 1;
        fetchSortedProducts(currentSort, page,currentTermId,currentTaxonomy);
        closeDrawer();
    });
    ...
    function fetchSortedProducts(sort, page,currentTermId,currentTaxonomy) {
       ...
       // alert(currentTermId);
        $.ajax({
            ...
            data: {
                ...
                sort: sort,
                page: page,   term_id: currentTermId ,taxonomy:currentTaxonomy
            }, 
    ...
    let currentTermId = <?php echo get_queried_object_id(); ?>;
       ...
       // alert(currentTermId);
        $.ajax({
            ...
            data: {
                ...
                age: selectedAge,     term_id: currentTermId, taxonomy: currentTaxonomy,
                page: page,
            },
            success: function(response) {
                if (append) {
                    const tempDiv = $('<div>').html(response);
                    $('.load-more-btn').closest('div.text-center').remove();
                    $('#product-grid').append(tempDiv.hide().fadeIn(400));
                } else {
                    $('#product-grid').html(response);
                }
