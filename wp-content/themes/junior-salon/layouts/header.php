...
<body <?php body_class(); ?> class="transition-opacity duration-300">
    ...
    <div class="bg-white w-full py-2 shadow-md" id="deal-bar">
        <div
            class="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center px-4 sm:px-6 lg:px-8 gap-2">
    ...
    <header class="sticky top-0 z-50 bg-white shadow-sm">
      ...
      if (has_custom_logo()) {
        ...
        $timestamp = file_exists($logo_path) ? filemtime($logo_path) : time();
        $logo_url_with_time = add_query_arg('v', $timestamp, $logo_url);
        ?>
                        <img src="<?php echo esc_url($logo_url_with_time); ?>"
                            alt="<?php echo esc_attr(get_bloginfo('name')); ?>" class="w-[180px] h-auto block"
                            loading="eager">
                        <?php
      } else {
        ?>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (dealEndDateTime) {
            ...
            const dealBar = document.getElementById(
                'deal-bar'); // Target the whole deal bar to hide after expiration
