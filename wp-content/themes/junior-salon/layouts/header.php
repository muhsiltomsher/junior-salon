...
<head>
  ...
  <!-- Preload Dynamic Logo if exists -->
  <?php
    if (has_custom_logo()) {
...
<body <?php body_class(); ?> class="transition-opacity duration-300">
          ...
          <a href="<?php echo esc_url(home_url('/')); ?>" class="block">
            <?php 
              if (has_custom_logo()) {
                ...
                echo '<img src="' . esc_url($logo_url_with_time) . '" alt="' . esc_attr(get_bloginfo('name')) . '" class="w-[180px] h-auto block">';
              } else {
                echo '<h1 class="text-2xl font-bold text-white">' . esc_html(get_bloginfo('name')) . '</h1>';
              }
            ?>
          </a>
        </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      ...
      if (dealEndDateTime) {
        const endTime = new Date(dealEndDateTime).getTime();
